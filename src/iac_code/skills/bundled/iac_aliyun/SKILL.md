...
### 权限错误（Permission denied / HTTP 401·403）
任何 `aliyun_api` 调用（含只读查询、Schema 查询、询价）返回授权被拒时，**禁止静默跳过继续后续步骤**。按顺序处置：

1. 告知用户这是权限问题，并说明缺少哪个云产品/API 的权限（错误文案中的 `<product>/<action>` 即所需授权范围）。
2. 尝试恢复：改用当前账号有权限的同语义替代 API，或用已有信息推进。
3. 无法恢复时显式标记该步骤失败，并在最终回复中写明这个错误和处置结论；不要把缺失的结果当成已获得。

**硬约束**：只要某个结果因权限被拒且未恢复，就不得继续执行依赖该结果的步骤（例如据此更新模板、判断可用规格、宣布目标完成）。
