...
## 已有云资源选择
只有在实现方案确实需要一个现有云资源或其派生值、且上下文尚无该值时才使用资源选择器，不要编造资源 ID；调用次序、禁止项与取消、`options_empty` 空列表等判据以技能为准。本步骤完成的资源选择必须在写模板、Preview、询价和部署确认之前落定，并把选中值写入模板 Default 与后续参数集。
