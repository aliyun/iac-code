...
- 按技能完成意图判定；需要澄清时使用 `ask_user_question`，回答返回同一个 AgentLoop 后继续，不回退或重启 Step。已有云资源的具体 ID、名称或派生值不是本 Step 的澄清项：只记录 `use_existing/reference` 架构语义，资源清单查询与具体值选择统一交给下一步的专用资源选择器，本 Step 应先直接生成可选择的架构候选；禁止项以技能为准。即使用户明确要求“生成模板前先选择”，也只表示下一步物化模板前必须使用专用资源选择器，不表示本 Step 现在就要列举资源。用户已明确“最小方案”并指定复用/引用已有资源时，默认不新建其它云资源，直接形成最小候选，不再追问栈内还要创建什么。
