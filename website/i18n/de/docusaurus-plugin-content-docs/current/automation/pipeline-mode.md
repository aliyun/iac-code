...
Der Pipeline-Modus sichert abgeschlossene Agent-Loop-Steps nicht mehr einzeln. Er veroeffentlicht zuerst `input_required` oder `waiting_input` und fuehrt danach ein einziges kritisches Backup aus; bei einem Fehler folgen `backup_blocked` und eine Pause im wiederherstellbaren Zustand. `pipeline_handoff_ready` und Endzustaende bleiben vor ihrer Publikation durch ein kritisches Backup geschuetzt. Fuer A2A-Beobachter folgt auf terminale und `pipeline_handoff_ready`-geschuetzte Publikationen ein `backup_committed`-Ereignis mit `committedEventId`, `committedEventType` und `committedSequence`, sobald der Mirror dauerhaft ist. Fortschritt aus `parallel_sub_pipeline` wird im naechsten Warte-, Handoff- oder Terminal-Backup erfasst, nicht in Checkpoints pro Step.
