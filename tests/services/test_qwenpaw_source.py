...
class TestReadActiveModel:
    def test_reads_from_providers_subdir(self, tmp_path):
        ...
        (providers / "active_model.json").write_text(json.dumps(data), encoding="utf-8")
    ...
    def test_reads_from_root_fallback(self, tmp_path):
        ...
        (tmp_path / "active_model.json").write_text(json.dumps(data), encoding="utf-8")
        assert _read_active_model(tmp_path) == data
    ...
    def test_prefers_providers_subdir_over_root(self, tmp_path):
        ...
        (providers / "active_model.json").write_text(json.dumps(subdir_data), encoding="utf-8")
        (tmp_path / "active_model.json").write_text(json.dumps(root_data), encoding="utf-8")
    ...
    def test_returns_none_for_invalid_json(self, tmp_path):
        providers = tmp_path / "providers"
        providers.mkdir()
        (providers / "active_model.json").write_text("not json", encoding="utf-8")
        (tmp_path / "active_model.json").write_text("also not json", encoding="utf-8")
        assert _read_active_model(tmp_path) is None
...
class TestReadProviderConfig:
    def test_reads_from_builtin_subdir(self, tmp_path):
        ...
        (builtin / "dashscope.json").write_text(json.dumps(data), encoding="utf-8")
    ...
    def test_reads_from_custom_subdir(self, tmp_path):
        ...
        (custom / "my-provider.json").write_text(json.dumps(data), encoding="utf-8")
    ...
    def test_reads_flat_fallback(self, tmp_path):
        ...
        (providers_dir / "dashscope.json").write_text(json.dumps(data), encoding="utf-8")
        assert _read_provider_config(tmp_path, "dashscope") == data
...
class TestLoadFromQwenPaw:
    ...
    def test_returns_config_with_provider_id_field(self, tmp_path, monkeypatch):
        ...
        (providers / "active_model.json").write_text(json.dumps(active), encoding="utf-8")
        builtin = providers / "builtin"
        builtin.mkdir()
        provider_cfg = {"api_key": "", "base_url": "https://custom.example.com/v1"}
        (builtin / "dashscope.json").write_text(json.dumps(provider_cfg), encoding="utf-8")
        monkeypatch.setenv("QWENPAW_SECRET_DIR", str(secret))
    ...
    def test_returns_config_with_legacy_provider_field(self, tmp_path, monkeypatch):
        ...
        (secret / "active_model.json").write_text(json.dumps(active), encoding="utf-8")
        providers_dir = secret / "providers"
        providers_dir.mkdir()
        provider_cfg = {"api_key": "", "base_url": "https://custom.example.com/v1"}
        (providers_dir / "dashscope.json").write_text(json.dumps(provider_cfg), encoding="utf-8")
        monkeypatch.setenv("QWENPAW_SECRET_DIR", str(secret))
