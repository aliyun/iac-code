    ...
    candidates:
      ...
      items:
        ...
        properties:
          ...
          monthly_estimate:
            type: string
            description: 月度费用粗估区间，口径固定为按量付费列表价折算月度（与 cost_estimating 询价口径一致），格式为 ¥<下限>-<上限>/月（按量付费列表价，架构规划粗估）；不得使用包年包月或合同优惠价口径，也不得省略"粗估"标注；区间必须与本方案 topology 中的实际规格自洽，最终价格以 cost_estimating 结果为准
          pros:
            type: array
            items:
              type: string
...
本步骤的 `monthly_estimate` 只是**粗估**，权威价格由后续 `cost_estimating` 通过 ROS 询价 API 给出。为了让两阶段可比：

- 口径固定为**按量付费列表价折算月度**，与 `cost_estimating` 的 `OriginalAmount` 口径一致。不要使用包年包月价格，也不要预估合同优惠价——折扣只能由询价 API 返回。
- 格式为 `¥<下限>-<上限>/月（按量付费列表价，架构规划粗估）`，必须保留「粗估」标注，避免被当作最终价格引用。
- 区间必须与本方案 `topology` 中写明的实际规格自洽：写了 1c1g 就不要给出 2c4g 的价格区间，否则模板生成阶段会与规划阶段双重偏离。
- 规格差异是方案差异的一部分，必须在 `topology` 中写明具体规格（如 `ecs.e-c1m1.large 2vCPU/2GiB`），供 `cost_estimating` 做规格对账。
...
- 费用估算基于阿里云公开定价的合理范围，不需要精确到个位；但必须使用按量付费列表价口径并标注为粗估
