...
已有云资源的具体 ID、名称或派生值不属于本步骤的架构澄清。用户已经明确要复用或引用某类资源时，直接记录对应的 `action: "use_existing"`/`"reference"` 并生成候选；具体资源统一交给下一步的专用资源选择器处理。即使用户明确要求“生成模板前先选择”，也只表示下一步物化模板前必须选择，不表示本步骤现在就要查询或展示资源。禁止在本步骤调用 `aliyun_api` 查询实例、密钥、Bucket 等资源清单，禁止把任何云资源候选塞进 `ask_user_question`，也禁止用自由输入索要资源 ID。资源是否存在、列表是否为空同样不在本步骤确认；本步骤应先完成架构候选展示。

用户明确要求“最小方案/最小模板”且已经说明要复用或引用的已有资源时，把“最小”解释为不额外新建其它云资源；直接生成仅包含该引用及实现模板所必需结构的最小候选，并把这一解释写成假设。不要再用 `ask_user_question` 追问栈内还要创建 ECS、OSS Bucket 或其它可选资源。只有用户同时给出了相互冲突的强约束，导致无法形成任何有效候选时，才需要澄清冲突。
