...
Comme un même état terminal logique peut apparaître plusieurs fois dans le flux d'événements, chaque publication protégée par sauvegarde porte un booléen explicite `authoritative` en plus de `visibility`. La pré-écriture `pending_backup` et le `pipeline_handoff_ready` dégradé (`visibility: "unavailable"`, `data.action: "switch_to_normal_unavailable"`) sont `authoritative: false`; seule la publication `committed` est `authoritative: true`. Les clients qui comptent les événements terminaux pour l'audit ou la supervision doivent fusionner par `(eventType, pipelineRunId)` et ne garder que les événements autoritatifs, sinon une seule annulation est comptée comme plusieurs `pipeline_canceled` et `pipeline_handoff_ready`. Les enveloppes sans ce champ sont antérieures et doivent se rabattre sur `visibility`.
