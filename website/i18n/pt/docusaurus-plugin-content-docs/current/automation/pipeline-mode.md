...
Voltar para o chat normal também exige que o pipeline tenha produzido seus resultados obrigatórios. Se uma etapa obrigatória ainda não produziu um resultado utilizável — por exemplo, quando a execução é cancelada enquanto a etapa de entendimento do requisito ainda está em andamento —, o IaC Code publica apenas o estado terminal e não emite o sinal de handoff, para que nenhum consumidor a jusante receba um resultado vazio como se o pipeline tivesse concluído seu trabalho.
...
## Checkpoints de backup

O modo Pipeline não faz mais backup de cada step concluído do agent loop. Ele publica primeiro `input_required` ou `waiting_input` e depois executa um único backup crítico; se falhar, publica `backup_blocked` e pausa em um estado recuperável. `pipeline_handoff_ready` e estados terminais continuam protegidos por um backup crítico antes da publicação. Para observadores A2A, publicações protegidas terminais e `pipeline_handoff_ready` são seguidas por um evento `backup_committed` com `committedEventId`, `committedEventType` e `committedSequence` quando o limite de backup do modo atual estiver durável. Com `IAC_CODE_CONFIG_BACKUP_TMP_DIR`, esse limite é o snapshot local imutável e permite recuperação no mesmo sandbox; a recuperação em outro sandbox depende da cópia em segundo plano para `IAC_CODE_CONFIG_BACKUP_DIR`. Sem preparação local, o limite continua sendo o diretório final. O progresso de `parallel_sub_pipeline` é capturado pelo próximo backup de espera, handoff ou terminal, não por checkpoints de cada step.
