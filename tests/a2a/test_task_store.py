...
@pytest.mark.asyncio
async def test_canceled_owner_release_pending_true_with_live_in_process_writer() -> None:
    ...
    record.active_task = active_writer
    store.mirror_task(record)

    try:
        assert await store.canceled_owner_release_pending(context_id="ctx-1", task_id="task-1") is True
    finally:
        release.set()
        await active_writer
...
@pytest.mark.asyncio
async def test_canceled_owner_release_pending_true_when_context_still_fenced(tmp_path) -> None:
    store = A2ATaskStore(metrics=NoOpA2AMetrics())
    context = await store.get_or_create_context(
        context_id="ctx-1",
        cwd=str(tmp_path),
...
@pytest.mark.asyncio
async def test_canceled_owner_release_pending_false_for_quiescent_cold_snapshot(tmp_path) -> None:
    persistence = A2APersistenceStore(tmp_path / "a2a")
    ...
    assert await store.canceled_owner_release_pending(context_id="ctx-1", task_id="task-1") is False
