...
Because a single logical terminal state can appear more than once in the event stream, every backup-gated publication carries an explicit `authoritative` boolean alongside `visibility`. The `pending_backup` pre-write and the degraded `pipeline_handoff_ready` fallback (`visibility: "unavailable"`, `data.action: "switch_to_normal_unavailable"`) are both `authoritative: false`; only the `committed` publication is `authoritative: true`. Clients that count terminal events for auditing or monitoring must merge by `(eventType, pipelineRunId)` and keep only authoritative events, otherwise one cancellation is counted as several `pipeline_canceled` and `pipeline_handoff_ready` events. Envelopes without the field predate it and should fall back to `visibility`.
