...
iac-code uses `TASK_STATE_INPUT_REQUIRED` as the normal completed state because the context remains available for follow-up messages. Ordinary A2A turns treat `TASK_STATE_INPUT_REQUIRED` as this normal completion state; their successful turn-end backup is the non-blocking `normal_turn_end` checkpoint. If an ordinary A2A terminal or cancellation publication is blocked by a critical backup gate, task metadata exposes `metadata.iac_code.backupBlocked` with `reason`, `error`, and `recoverable`; clients should wait for recovery before sending the next turn. Pipeline mode publishes backup failures as pipeline events, so clients should inspect `metadata.iac_code.pipeline.eventType == "backup_blocked"` and the event data. Pipeline mode no longer creates completed-step backups; `pipeline_step_completed` is retained only for legacy recovery. With `IAC_CODE_CONFIG_BACKUP_DIR` enabled, `input_required` and `waiting_input` are published before their single critical backup runs. The `terminal` reason still gates terminal publications, and the `handoff_ready` reason still gates the `pipeline_handoff_ready` event type. For terminal and `pipeline_handoff_ready` committed publications, iac-code persists a `backup_committed` pipeline event whose data contains `committedEventId`, `committedEventType`, and `committedSequence`; clients can pair it with the protected event before treating the publication as recoverable after restart.

When `IAC_CODE_CONFIG_BACKUP_TMP_DIR` is enabled for the A2A server, `backup_committed` confirms the local immutable snapshot and supports recovery within the same sandbox; it does not confirm that the final backup directory has been updated. Cross-sandbox recovery becomes available after the background publisher copies that snapshot to `IAC_CODE_CONFIG_BACKUP_DIR`. Without staging, `backup_committed` continues to confirm the final backup directory.
...
## Resource Selector Extension

`urn:iac-code:resource-selector:v1` is an optional extension for selecting one cloud resource or one value derived from it. The Agent Card advertises it only when `IAC_CODE_A2A_RESOURCE_SELECTOR_ENABLED` is enabled. A client opts in per request with `metadata.iac_code.capabilities.resourceSelector`, whose value must contain `schemaVersion: 1`, `queryMode: "ros_api_json"`, and the `profileHash` advertised by the selector capability. Clients must read that hash from the actually loaded selector bundle; they must not hard-code it.

When the model requests a selection, the server enters `input-required` and emits the selector contract. The client queries and renders the selector, then resumes with a structured confirm or cancel response. Credentials and secrets are never included in selector event payloads. Safe Mode retains the selector tools, but the environment flag, client capability, matching profile hash, and an effective Alibaba Cloud credential are still required.
