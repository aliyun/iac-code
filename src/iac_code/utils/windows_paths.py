...
On Windows, iac-code runs the shell tool through Git for Windows' bash. The
model may emit POSIX-form paths (``/c/Users/foo``) to the file tools, but
...
def posix_path_to_windows(path: str) -> str:
    ...
    if len(path) >= 2 and path[0] == "/" and path[1].isalpha() and (len(path) == 2 or path[2] == "/"):
        drive = path[1].upper()
