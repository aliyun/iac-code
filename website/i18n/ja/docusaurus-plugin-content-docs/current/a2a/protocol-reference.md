...
同じ論理的な終端状態が event stream に複数回現れることがあるため、backup で保護されるすべての publication は `visibility` に加えて明示的な `authoritative` boolean を持ちます。`pending_backup` の事前書き込みと、縮退した `pipeline_handoff_ready`（`visibility: "unavailable"`、`data.action: "switch_to_normal_unavailable"`）はいずれも `authoritative: false` で、`committed` publication だけが `authoritative: true` です。監査や監視のために terminal event を数える client は `(eventType, pipelineRunId)` で統合し、authoritative な event だけを保持してください。そうしないと 1 回のキャンセルが複数の `pipeline_canceled` と `pipeline_handoff_ready` として数えられます。このフィールドがない envelope は以前のバージョンのものなので、`visibility` にフォールバックしてください。
