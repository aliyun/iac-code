...
- 默认情况下，工具权限请求会以结构化的 `input-required` 等待暂停，调用方可用 `allow_once` 或 `deny` 恢复。`auto-approve-permissions` 或显式权限规则可以让请求无需等待便得到处理。权限决策会在本地审计；任何需要审计记录的允许决策在审计记录无法持久化时都会 fail closed。在非 blanket bypass 模式下，受保护的阿里云写 API 需要按 API 精确授权。
