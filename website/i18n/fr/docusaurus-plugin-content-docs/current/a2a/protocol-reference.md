...
iac-code utilise `TASK_STATE_INPUT_REQUIRED` comme état terminé normal, car le contexte reste disponible pour les messages de suivi. Les tours A2A ordinaires traitent `TASK_STATE_INPUT_REQUIRED` comme cet état de fin normal; leur sauvegarde réussie de fin de tour est le checkpoint non bloquant `normal_turn_end`. Si une publication terminale ou d'annulation A2A ordinaire est bloquée par une barrière de sauvegarde critique, les métadonnées de tâche exposent `metadata.iac_code.backupBlocked` avec `reason`, `error` et `recoverable`; les clients doivent attendre la récupération avant d'envoyer le tour suivant. Le mode Pipeline publie les échecs de sauvegarde comme événements pipeline; les clients doivent donc inspecter `metadata.iac_code.pipeline.eventType == "backup_blocked"` et les données de l'événement. Le mode Pipeline ne crée plus de sauvegarde après chaque step; `pipeline_step_completed` est conservé uniquement pour la récupération historique. Avec `IAC_CODE_CONFIG_BACKUP_DIR`, `input_required` et `waiting_input` sont publiés avant leur unique sauvegarde critique. La raison `terminal` continue de protéger les publications terminales, et `handoff_ready` continue de protéger `pipeline_handoff_ready`. Pour les publications committed terminales et `pipeline_handoff_ready`, iac-code persiste un événement pipeline `backup_committed` dont les données contiennent `committedEventId`, `committedEventType` et `committedSequence`; les clients peuvent l'apparier avec l'événement protégé avant de considérer la publication comme récupérable après redémarrage.

Quand le serveur A2A utilise `IAC_CODE_CONFIG_BACKUP_TMP_DIR`, `backup_committed` confirme le snapshot local immuable et permet la reprise dans le même sandbox; il ne confirme pas la mise à jour du répertoire final. La reprise dans un autre sandbox devient possible après la copie du snapshot vers `IAC_CODE_CONFIG_BACKUP_DIR` par le processus en arrière-plan. Sans préparation locale, `backup_committed` continue de confirmer le répertoire final.
...
## Extension du sélecteur de ressources

`urn:iac-code:resource-selector:v1` est une extension facultative permettant de sélectionner une seule ressource cloud ou une valeur qui en dérive. L'Agent Card ne l'annonce que si `IAC_CODE_A2A_RESOURCE_SELECTOR_ENABLED` est activée. Le client déclare sa prise en charge dans chaque requête via `metadata.iac_code.capabilities.resourceSelector`, dont la valeur doit contenir `schemaVersion: 1`, `queryMode: "ros_api_json"` et le `profileHash` annoncé par la capacité du sélecteur. Ce hash doit provenir du bundle réellement chargé et ne doit pas être codé en dur.

Lorsque le modèle demande une sélection, le serveur passe à l'état `input-required` et émet le contrat du sélecteur. Le client interroge et affiche le sélecteur, puis reprend la tâche avec une réponse structurée de confirmation ou d'annulation. Les identifiants et secrets ne figurent jamais dans les événements du sélecteur. Safe Mode conserve les outils, mais le paramètre serveur, la capacité client, un profile hash identique et des identifiants Alibaba Cloud effectifs restent obligatoires.
