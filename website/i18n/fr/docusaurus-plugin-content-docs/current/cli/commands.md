...
| `/memory` | Ouvrir le sélecteur de mémoire. Modifiez les fichiers `IAC-CODE.md` de projet ou d'utilisateur, activez ou désactivez auto-memory, et ouvrez le dossier auto-memory du projet lorsque auto-memory est activé. |
...
- La mémoire de projet est enregistrée dans `IAC-CODE.md` à la racine du projet.
- La mémoire utilisateur est enregistrée dans `IAC-CODE.md` dans le répertoire de configuration d'exécution, `~/.iac-code/` par défaut.
