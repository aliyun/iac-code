...
- 新增 `tests/repl_e2e/test_run_pipeline_scenarios.py`，覆盖 runner 的参数校验、脱敏、scenario dispatch、acceptance check、自动 teardown 等纯 helper 逻辑；该 pytest 不启动真实 REPL、不调用真实 LLM 或云账号。
