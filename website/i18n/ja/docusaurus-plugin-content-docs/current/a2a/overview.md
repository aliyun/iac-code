...
| **外部 Skill ブリッジ** | `python3 scripts/iac_code.py start --follow` | 外部エージェントが iac-code を Alibaba Cloud インフラ機能として組み込む |
...
- **リクエスト単位の言語** — 呼び出し側が `metadata.iac_code.preferredLanguage` を宣言すると、ユーザーに表示されるテキストがリクエスト単位でローカライズされ、並行タスクがグローバル locale を取り合いません。
- **対話的な権限決定** — 権限リクエストはターンを一時停止して構造化された権限エンベロープを発行し、呼び出し側がサイドバンドメッセージで `allow_once` または `deny` と応答します。
...
A2A サーバーモードでは、既定の構成でツール権限リクエストが対話的な入力待ちに変換されます。ターンは一時停止して構造化された権限エンベロープを発行し、呼び出し側がサイドバンドメッセージで `allow_once` または `deny` と応答します。`auto-approve-permissions` または明示的な権限ルールが有効な場合は、代わりに自動承認またはルールによる裁定が行われます。権限決定はローカルで監査され、監査レコードを必要とする allow 決定は、そのレコードを永続化できない場合に fail-closed になります。保護された Alibaba Cloud 書き込み API は、グローバル bypass モード以外では引き続き API ごとの正確な承認が必要です。認証なしの A2A モードは信頼できるローカル環境でのみ実行するか、Bearer token、Basic auth、または API key 認証で保護してください。
