...
Para elegir una arquitectura antes de generar su plantilla:
...
| Pipeline `selling` | Genera y evalúa plantillas candidatas antes de que el usuario elija una para desplegar. Sigue siendo el predeterminado. |
| Pipeline `selling_solution_first` | Permite elegir primero una arquitectura y después genera, previsualiza y calcula el precio solo de esa solución. |

Seleccione cualquiera de los dos con `IAC_CODE_PIPELINE_NAME`. Consulte [Pipeline de solución primero](./solution-first-pipeline.md) para conocer el flujo de tres etapas, los límites separados de confirmación y permiso, y la recuperación.
...
Al integrar el pipeline mediante A2A, los llamadores también pueden declarar dos capacidades en los metadatos del mensaje: `metadata.iac_code.preferredLanguage` hace que el progreso, las preguntas y los avisos de permisos se devuelvan en el idioma preferido del llamador; `metadata.iac_code.candidatePresentation: rich-v1` hace que el paso de confirmación del plan devuelva una carga estructurada (nombre del candidato, resumen, diagrama de arquitectura, coste mensual total, partidas de coste) apta para renderizado enriquecido en interfaces externas. Cuando un despliegue u operación de herramienta requiere permiso, el pipeline publica un sobre de permiso pendiente y los llamadores responden con `allow_once` o `deny` mediante un mensaje de banda lateral; consulta la [Referencia del protocolo](../a2a/protocol-reference.md) para más detalles.
...
- La versión actual incluye `selling` y `selling_solution_first`, ambos orientados principalmente a flujos de infraestructura de Alibaba Cloud. `selling` sigue siendo el predeterminado.
...
El modo Pipeline no crea backups al completar steps del agent loop. Primero publica `input_required` o `waiting_input` y después ejecuta un único backup crítico; si falla, publica `backup_blocked` y se pausa en un estado recuperable. `pipeline_handoff_ready` y los estados terminales siguen protegidos por un backup crítico previo a su publicación. Para observadores A2A, las publicaciones protegidas terminales y `pipeline_handoff_ready` van seguidas de un evento `backup_committed` con `committedEventId`, `committedEventType` y `committedSequence` cuando el límite de backup del modo actual ya es durable. Con `IAC_CODE_CONFIG_BACKUP_TMP_DIR`, ese límite es el snapshot local inmutable y permite recuperar en el mismo sandbox; la recuperación en otro sandbox depende de que el proceso en segundo plano lo copie a `IAC_CODE_CONFIG_BACKUP_DIR`. Sin preparación local, el límite sigue siendo el directorio final. El progreso de `parallel_sub_pipeline` se captura en el siguiente backup de espera, handoff o terminal, no en checkpoints por step.
