# Network Architecture InfraGuard Usage Guidelines
...
The scenario pack in `../packs/iac-code-network-architecture-pack.rego` lists the rule ids enforced for this scenario. Rules use `package infraguard.rules.aliyun.<snake_case>`, `rule_meta`, and `deny contains result if` in the official InfraGuard style.
