...
from iac_code.a2a.parts import parts_to_user_input
...
def test_parts_to_user_input_converts_raw_image(monkeypatch, tmp_path) -> None:
    raw = b"fake png bytes"
    resized = b"resized raw image"
    calls = _resize_spy(monkeypatch, output=resized, media_type="image/webp")

    value = parts_to_user_input([Part(raw=raw, media_type="image/png")], cwd=tmp_path)
...
def test_parts_to_user_input_preserves_text_plus_image_order(monkeypatch, tmp_path) -> None:
...
def test_parts_to_user_input_converts_base64_data_image(monkeypatch, tmp_path) -> None:
    ...
    value = parts_to_user_input([_binary_data_part({"bytes": encoded}, media_type="image/png")], cwd=tmp_path)

    assert calls == [raw]
    assert value.content == [ImageBlock(media_type="image/png", data=base64.b64encode(resized).decode("ascii"))]


def test_parts_to_user_input_converts_safe_file_url_image(monkeypatch, tmp_path) -> None:
    ...
    value = parts_to_user_input([Part(url=source.as_uri(), media_type="image/png")], cwd=tmp_path)

    assert calls == [raw]
    assert value.content == [ImageBlock(media_type="image/png", data=base64.b64encode(resized).decode("ascii"))]


def test_parts_to_user_input_uses_real_resizer_for_valid_image_bytes(tmp_path) -> None:
    raw = _tiny_bmp_bytes()
...
def test_parts_to_user_input_accepts_tiny_png_without_monkeypatch(tmp_path) -> None:
    raw = _tiny_png_bytes()
...
def test_parts_to_user_input_rejects_unsafe_file_url_image(tmp_path) -> None:
    outside = tmp_path.parent / "outside-diagram.png"
    outside.write_bytes(b"outside")

    with pytest.raises(ValueError, match="outside the allowed workspace"):
        parts_to_user_input([Part(url=outside.as_uri(), media_type="image/png")], cwd=tmp_path)


def test_parts_to_user_input_rejects_invalid_base64_data_image(tmp_path) -> None:
    with pytest.raises(ValueError, match="valid base64"):
        parts_to_user_input([_binary_data_part({"bytes": "not-base64!"}, media_type="image/png")], cwd=tmp_path)


def test_parts_to_user_input_rejects_oversized_raw_image(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr("iac_code.a2a.parts.MAX_BINARY_INLINE_BYTES", 3)

    with pytest.raises(ValueError, match="too large"):
        parts_to_user_input([Part(raw=b"abcd", media_type="image/png")], cwd=tmp_path)


def test_parts_to_user_input_rejects_audio_as_true_image(tmp_path) -> None:
    with pytest.raises(ValueError, match="unsupported image media type"):
        parts_to_user_input([Part(raw=b"audio", media_type="audio/wav")], cwd=tmp_path)
