...
Como um mesmo estado terminal lógico pode aparecer várias vezes no fluxo de eventos, toda publicação protegida por backup carrega um booleano explícito `authoritative` além de `visibility`. A pré-escrita `pending_backup` e o `pipeline_handoff_ready` degradado (`visibility: "unavailable"`, `data.action: "switch_to_normal_unavailable"`) são `authoritative: false`; apenas a publicação `committed` é `authoritative: true`. Clientes que contam eventos terminais para auditoria ou monitoramento devem mesclar por `(eventType, pipelineRunId)` e manter apenas eventos autoritativos, caso contrário um único cancelamento é contado como vários `pipeline_canceled` e `pipeline_handoff_ready`. Envelopes sem esse campo são anteriores e devem recorrer a `visibility`.
