...
`/` を入力すると、利用可能なスラッシュコマンドを確認できます。よく使う運用コマンドには、現在のセッション状態を表示する `/status`、スキル管理の `/skills`、プロジェクトとユーザーのメモリファイルを扱う `/memory`、アクティブなセッションに名前を付ける `/rename`、セッションを切り替える `/resume` があります。
