...
| `IAC_CODE_A2A_RESOURCE_SELECTOR_ENABLED` | A2A Server only. Enables resource-selector capability advertisement and registration when set to `1`, `true`, `yes`, or `on` (case-insensitive). Unset, empty, or any other value disables it; default: disabled |
...
| `IAC_CODE_CONFIG_BACKUP_TMP_DIR` | Optional local staging directory for `iac-code a2a`; requires `IAC_CODE_CONFIG_BACKUP_DIR`. A2A backups block only until an immutable `<session_id>_vX` snapshot is complete here, then a dedicated process copies snapshots to the final backup directory in version order and removes them after success. Both paths must be absolute and non-overlapping, and this directory must be outside `IAC_CODE_CONFIG_DIR`. Other run modes ignore this setting. |
