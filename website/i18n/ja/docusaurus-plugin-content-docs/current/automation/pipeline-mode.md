...
通常チャットに戻るには、パイプラインが必須の成果物を産出していることも条件になります。必須ステップがまだ利用可能な結果を産出していない場合（たとえば要件理解の段階が実行中のまま実行がキャンセルされた場合）、IaC Code は終了状態のみを発行し、ハンドオフ準備完了のシグナルは発行しません。これにより、下流の利用者が空の結果をパイプライン完了として受け取ることを防ぎます。
