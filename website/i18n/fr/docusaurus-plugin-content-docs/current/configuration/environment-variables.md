...
| `IAC_CODE_A2A_RESOURCE_SELECTOR_ENABLED` | A2A Server uniquement. Active l'annonce et l'enregistrement du sélecteur de ressources avec `1`, `true`, `yes` ou `on` (sans distinction de casse). Une valeur absente, vide ou différente le désactive ; désactivé par défaut |
...
| `IAC_CODE_CONFIG_BACKUP_TMP_DIR` | Répertoire local de préparation optionnel pour `iac-code a2a`; nécessite `IAC_CODE_CONFIG_BACKUP_DIR`. Les sauvegardes A2A ne bloquent que jusqu’à la création locale d’un snapshot immuable `<session_id>_vX`; un processus dédié copie ensuite les snapshots vers le répertoire final dans l’ordre des versions et les supprime après réussite. Les deux chemins doivent être absolus et sans chevauchement, et ce répertoire doit se trouver hors de `IAC_CODE_CONFIG_DIR`. Les autres modes ignorent ce réglage. |
