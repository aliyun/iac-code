...
pub fn fernet_decrypt(fernet_key: &str, token: &str) -> Result<Vec<u8>, SecretCryptoError> {
    let key = FernetKey::decode(fernet_key)?;
...
impl FernetKey {
    fn decode(value: &str) -> Result<Self, SecretCryptoError> {
        let key = decode_base64_url(value)?;
        if key.len() != FERNET_KEY_LEN {
            return Err(SecretCryptoError::new(
        ...
        Ok(Self {
            signing_key: key[..FERNET_SIGNING_KEY_LEN]
                .try_into()
                .map_err(|_| SecretCryptoError::new("A2A push secret keyring is malformed"))?,
...
fn decode_base64_url(value: &str) -> Result<Vec<u8>, SecretCryptoError> {
    let normalized = normalize_base64_padding(value);
    URL_SAFE
        .decode(normalized.as_bytes())
        .map_err(|_| SecretCryptoError::new("A2A push secret ciphertext could not be decrypted"))
}
...
fn decrypt_error() -> SecretCryptoError {
    SecretCryptoError::new("A2A push secret ciphertext could not be decrypted")
}
