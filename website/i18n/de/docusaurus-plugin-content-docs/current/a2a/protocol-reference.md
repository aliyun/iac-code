...
Da derselbe logische Endzustand mehrfach im Ereignisstrom auftreten kann, tragen alle backup-geschuetzten Publikationen neben `visibility` einen expliziten booleschen Wert `authoritative`. Das `pending_backup`-Vorschreiben und das herabgestufte `pipeline_handoff_ready` (`visibility: "unavailable"`, `data.action: "switch_to_normal_unavailable"`) sind beide `authoritative: false`; nur die `committed`-Publikation ist `authoritative: true`. Clients, die Endzustandsereignisse fuer Audit oder Monitoring zaehlen, muessen nach `(eventType, pipelineRunId)` zusammenfassen und nur autoritative Ereignisse behalten, sonst wird ein einzelner Abbruch als mehrere `pipeline_canceled` und `pipeline_handoff_ready` gezaehlt. Umschlaege ohne dieses Feld stammen aus aelteren Versionen und sollten auf `visibility` zurueckfallen.
