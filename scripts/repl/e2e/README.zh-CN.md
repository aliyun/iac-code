...
默认写入 `/tmp/iac-code-repl-e2e-runs/<scenario>/<timestamp>-<pid>-<id>/`：
...
uv run python scripts/repl/e2e/run_pipeline_scenarios.py \
  ...
  --run-dir /tmp/iac-code-repl-e2e-selection
```
...
- `--cleanup-continue-prompt` 指定 `rollback-step5-cleanup-recovery` 在 `--continue` 恢复后用于继续 cleanup 的输入；默认 `继续`。
