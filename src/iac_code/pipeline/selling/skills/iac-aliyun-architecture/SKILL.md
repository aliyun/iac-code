    ...
    candidates:
      ...
      items:
        ...
        properties:
          ...
          monthly_estimate:
            type: string
            description: 架构阶段粗估口径的月度费用范围；基于阿里云公开定价的量级估算，通常按量计费且不含系统盘、带宽等全量项，仅供方案量级对比。最终月成本以 cost_estimating 步骤的 ROS 全量询价为准，两者口径不同、可能存在数倍差异。
          pros:
            type: array
            items:
              type: string
...
- `monthly_estimate` 是**架构粗估口径**：基于公开定价的量级估算，通常按量计费且不含系统盘、带宽等全量项，仅供方案量级对比。它与 cost_estimating 步骤的 ROS 全量询价口径不同（后者含系统盘、带宽等，且区分列表价/合同优惠价），两者可能存在数倍差异。最终月成本以 cost_estimating 的 ROS 询价为准，不要把本粗估当作最终报价。
